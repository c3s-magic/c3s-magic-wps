...
class RainFARM(Process):
    def __init__(self):
        inputs = [
            *model_experiment_ensemble(start_end_year=(1850, 2005), start_end_defaults=(1997, 1997)),
            ...
            LiteralInput(
                'target_grid',
                'Target Grid',
                abstract='in degrees (e.g. \'1x1\') can also be the name of one of the datasets to use the grid from that dataset',
                data_type='string',
                default='1x1',
            ),
            LiteralInput(
            ...
            LiteralInput(
                'nf',
                'n Subdivisions',
                abstract='number of subdivisions for downscaling (e.g. 8 will produce output fields with linear resolution increased by a factor 8)',
                data_type='postiveInteger',
                default=8,
            ),
            LiteralInput(
                'conserv_glob',
                'Conserve Global',
                abstract='to conserve precipitation over full domain',
                data_type='string',
            ...
            LiteralInput(
                'weights_climo',
                'Weights Climo',
                abstract='if to conserve precipitation over full domain',
                data_type='string',
        ...
        super(RainFARM, self).__init__(
            ...
            The selected region needs to have equal and even number of longitude (in any case it is cut) and latitude grid points (e.g., 2x2, 4x4, ...).
    ...
    def _handler(self, request, response):
        ...
        response.outputs['archive'].file = runner.compress_output(os.path.join(workdir, 'output'), 'rainfarm_result.zip')
